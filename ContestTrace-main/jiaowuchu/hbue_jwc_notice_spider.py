...
列表：https://jwc.hbue.edu.cn/704/list1.htm（分页 list2.htm、list3.htm…）

调度示例：
  python d:\\xiangmu\\ConTest11\\jiaowuchu\\hbue_jwc_notice_spider.py --mode incremental

全量：
  python hbue_jwc_notice_spider.py --mode full
...
BASE_URL = "https://jwc.hbue.edu.cn"
# 教务处：首页为 list1.htm（无 list.htm），翻页 list2、list3…
LIST_PREFIX = "/704/list"
SOURCE_DEFAULT = "教务处"
CATEGORY = "通知公告"
...
def list_page_url(page: int) -> str:
    p = max(1, page)
    return f"{BASE_URL}{LIST_PREFIX}{p}.htm"


def parse_list_page(html: str) -> tuple[list[ListItem], Optional[str]]:
    """教务处列表多为 ul.wp_article_list / li.list_item；兼容 ul.news_list。"""
    soup = BeautifulSoup(html, "lxml")
    items: list[ListItem] = []
    base_host = urlparse(BASE_URL).netloc

    def same_site(abs_url: str) -> bool:
        host = urlparse(abs_url).netloc
        return not host or host == base_host

    ul = soup.select_one("ul.wp_article_list")
    if ul and ul.select("li.list_item"):
        for li in ul.select("li.list_item"):
            a = li.select_one("span.Article_Title a") or li.select_one("a")
            meta = li.select_one("span.Article_PublishDate")
        ...
        if not ul:
            log.error("列表页未找到 ul.wp_article_list 或 ul.news_list")
            return items, None
...
def crawl(
    ...
    *,
    ...
    if purge_before:
        log.warning("已开启 purge_before：将删除 notices 表中 spider_name=%s 的全部记录", SPIDER_NAME)
        conn.execute("DELETE FROM notices WHERE spider_name = ?", (SPIDER_NAME,))
        conn.commit()
...
def main() -> None:
    ap = argparse.ArgumentParser(description="湖北经济学院教务处通知公告爬虫")
    ...
    ap.add_argument(
        "--purge-before",
        action="store_true",
        help="重爬前先删除 notices 表中该爬虫的历史数据（按 spider_name 匹配）",
    )
    args = ap.parse_args()
    crawl(args.mode, args.db, args.max_pages, purge_before=args.purge_before)
