# -*- coding: utf-8 -*-
"""
针对湖北经济学院会计学院 · 通知公告（已入库的 title + content）提取并回写字段，不请求网络。
...
同时更新 updated_at。默认仅处理 spider_name = hbue_kjxy_notice_spider 的记录。
...
from hbue_kjxy_notice_spider import DEFAULT_DB_PATH, SPIDER_NAME
...
# ---------------------------------------------------------------------------
# 主题标签（会计学院通知常见主题）：(标签名, 触发词列表)
# ---------------------------------------------------------------------------

TAG_RULES: list[tuple[str, tuple[str, ...]]] = [
    ("团学党建", ("团支部", "党员", "入党", "发展对象", "预备党员", "党建")),
    ("评选公示", ("公示", "拟推荐", "初评", "评审结果", "评选结果", "五四")),
    ("奖学金资助", ("奖学金", "助学金", "国家奖学金", "励志")),
    ("教学教务", ("教学", "课程", "考试", "培养方案", "选课", "学风")),
    ("专业证书", ("ACCA", "CPA", "审计", "初级会计")),
    ("反诈安全", ("反诈", "诈骗", "安全")),
    ("活动通知", ("举办", "开展", "主题", "活动")),
    ("材料申报", ("申报", "提交材料", "报名表")),
