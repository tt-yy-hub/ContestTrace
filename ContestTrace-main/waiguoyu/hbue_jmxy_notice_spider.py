# -*- coding: utf-8 -*-
"""
湖北经济学院外国语学院（商务英语学院） · 学生公告爬虫

站点入口（分页：list.htm、list2.htm...）：
  https://wyxy.hbue.edu.cn/541/list.htm

采集策略：
- 全量：仅入库发布时间在 2025-01-01 至 2026-04-09（含边界）的通知
- 增量：每周运行一次，仅抓发布时间晚于库中已有最大 publish_time 的条目
- 唯一约束：url UNIQUE；重复使用 INSERT OR IGNORE 跳过

合规：
- robots.txt 曾 404，仍需自觉限流（列表与详情之间约 1s）

用法：
  python hbue_jmxy_notice_spider.py --mode full
  python hbue_jmxy_notice_spider.py --mode incremental
  python hbue_jmxy_notice_spider.py --mode full --max-pages 1
"""
...
BASE_URL = "https://wyxy.hbue.edu.cn"
LIST_PREFIX = "/541/list"
SOURCE_DEFAULT = "外国语学院"
CATEGORY = "通知公告"

# 兼容历史/统一命名：按你的约定固定该 spider_name
SPIDER_NAME = "hbue_jmxy_notice_spider"
...
def parse_publish_time_from_detail_html(
    soup: BeautifulSoup, plain_text: str, list_fallback: Optional[date]
) -> tuple[str, bool]:
    """
...
# 文档类链接与图片按「文本 绝对URL」写入，缺漏的在文末【文档链接】区块补齐
# ---------------------------------------------------------------------------
...
def parse_list_page(html: str) -> tuple[list[ListItem], Optional[str]]:
    ...
    # 站点分页常见形态：`<a class="next" ...>` 或 文本为“下一页>>”的普通链接
    nxt = soup.select_one("a.next")
    if nxt and nxt.get("href"):
        h = (nxt.get("href") or "").strip()
...
def parse_detail(html: str, list_item: ListItem) -> Optional[dict]:
    ...
    publish_time, used_fallback = parse_publish_time_from_detail_html(soup, plain_for_meta, list_item.list_date)
...
def crawl(mode: str, db_path: Path, max_pages: Optional[int] = None) -> None:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="外国语学院学生公告爬虫")
