...
# -*- coding: utf-8 -*-
"""
更新 data 目录下所有数据库中的所有记录的 confidence 字段，包括非竞赛公告
"""
...
def update_database_confidence_full(db_path):
    """
    更新指定数据库中所有表的所有记录的 confidence 字段
    """
    try:
        # 初始化过滤器
        ...
        for table in tables:
            ...
            if 'title' in column_names and 'content' in column_names and 'confidence' in column_names:
                ...
                if records:
                    logger.info(f"  处理表 {table_name}，共 {len(records)} 条记录（包括非竞赛公告）")
...
def main():
    ...
    for db_file in db_files:
        ...
        update_database_confidence_full(db_path)
